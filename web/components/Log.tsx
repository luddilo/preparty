...
const DELAY_BETWEEN_CHARACTERS = 10
...
class Log extends React.Component<LogProps, any> {
  ...
  getTextMessage = (message: RichMessage, last: boolean) => {
    ...
    if (last) { // Last of the current batch (i.e not the same as this.props.last which is true if the batch is the final batch)
      if (message.fromUser) {
        ...
        if (this.state.suggestionsAllowed) {
          this.setState({
            ...this.state,
            suggestionsAllowed: false,
          })
        ...
        if (message.customEvent) {
          // Handle event through the passed down function
        ...
        return (
          <Typist // Animating text being typed
            ref="typist"
  ...
  getSuggestions = (message, handleClick) => {
    return (
  ...
  render() {
    ...
    return (
      <div ref="log" {...props} className={"top"}>
        ...
        {this.state.suggestionsAllowed &&
          messages &&
          messages.length > 0 &&
          messages[messages.length - 1].suggestions &&
          this.getSuggestions(messages[messages.length - 1], this.props.handleClick)}
        <style jsx>{`
          .top {
